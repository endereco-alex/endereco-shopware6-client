...
for version in "${versions[@]}"; do
    ...
    if [ -d "./shops/$version/vendor" ]; then
        echo "Skipping version $version, already fetched."
        continue
    fi
    ...
    # Extract the archive (GitHub creates a folder like "production-6.4.0.0")
    echo "Extracting archive..."
    unzip -q "$temp_zip" -d "/tmp"

    # Move files from extracted folder to shops directory
    extracted_folder="/tmp/production-${version}"
